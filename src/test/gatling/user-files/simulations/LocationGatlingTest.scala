...
class LocationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Location entity")
        ...
        .repeat(2) {
            ...
            .get("/awssalaunch/api/locations")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new location")
            .post("/awssalaunch/api/locations")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "seaQuadrant":"SAMPLE_TEXT"
                , "friendlys":"SAMPLE_TEXT"
                , "hostiles":"SAMPLE_TEXT"
                , "status":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            ...
            .repeat(5) {
                ...
                .get("/awssalaunch${new_location_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created location")
            .delete("/awssalaunch${new_location_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
